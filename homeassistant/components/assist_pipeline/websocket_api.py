...
async def websocket_run(
    ...
    """Run a pipeline."""
    pipeline = await _get_pipeline(hass, connection, msg)
    if pipeline is None:
        return
    prepared = await _prepare_pipeline_input(hass, pipeline, connection, msg)
    if prepared is None:
        return
    pipeline_input, unregister_handler = prepared
...
async def _prepare_pipeline_input(
    ...
    connection: websocket_api.ActiveConnection,
    msg: dict[str, Any],
) -> tuple[PipelineInput, Callable[[], None] | None] | None:
    ...
    if start_stage == PipelineStage.WAKE_WORD:
        ...
        audio_settings, handler_id, unregister_handler = _setup_audio_pipeline(
            pipeline, connection, input_args, msg
        )
    elif start_stage == PipelineStage.STT:
        audio_settings, handler_id, unregister_handler = _setup_audio_pipeline(
            pipeline, connection, input_args, msg
        )
        input_args["wake_word_phrase"] = msg["input"].get("wake_word_phrase")
    ...
    try:
        ...
        return None
    return pipeline_input, unregister_handler


def _setup_audio_pipeline(
    pipeline: Pipeline,
    connection: websocket_api.ActiveConnection,
    input_args: dict[str, Any],
    msg: dict[str, Any],
) -> tuple[AudioSettings, int, Callable[[], None]]:
    ...
    audio_settings = AudioSettings(
        noise_suppression_level=msg["input"].get("noise_suppression_level", 0),
        auto_gain_dbfs=msg["input"].get("auto_gain_dbfs", 0),
        volume_multiplier=msg["input"].get("volume_multiplier", 1.0),
        is_vad_enabled=not msg["input"].get("no_vad", False),
    )
    handler_id, unregister_handler = connection.async_register_binary_handler(
        handle_binary
    )
    return audio_settings, handler_id, unregister_handler
