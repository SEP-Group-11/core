...
class AlexaThermostatController(AlexaCapability):
    ...
    def get_property(self, name: str) -> Any:
        ...
        if name in self.TEMP_ATTRIBUTES:
            return self._get_temperature(name)
        raise UnsupportedProperty(name)

    def _get_thermostat_mode(self) -> str | None:
        """Return the thermostat mode if valid or applicable."""
    ...
    def _get_temperature(self, setpoint: str) -> dict[str, Any] | None:
        """Return the temperature if valid or applicable."""
        temp = self.entity.attributes.get(self.TEMP_ATTRIBUTES[setpoint])
        if temp is None:
            return None
...
class AlexaModeController(AlexaCapability):
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        ...
        handlers = {
            ...
            f"{humidifier.DOMAIN}.{humidifier.ATTR_MODE}": self._handle_capability_resources_humidifier_mode,
            f"{water_heater.DOMAIN}.{water_heater.ATTR_OPERATION_MODE}": self._handle_capability_resources_water_heater_mode,
    ...
    def _handle_capability_resources_humidifier_mode(
        self,
    ...
    def _handle_capability_resources_water_heater_mode(
        self,
    ) -> dict[str, list[dict[str, Any]]]:
...
class AlexaRangeController(AlexaCapability):
    ...
    def _get_fan_percentage(self) -> float:
        ...
        if supported and fan.FanEntityFeature.SET_SPEED:
            return cast(float, self.entity.attributes.get(fan.ATTR_PERCENTAGE))
    ...
    def _get_humidifier_humidity(self) -> float:
        ...
        return cast(float, self.entity.attributes.get(humidifier.ATTR_HUMIDITY, 0))
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        ...
        handlers = {
            ...
            f"{cover.DOMAIN}.{cover.ATTR_POSITION}": self._cover_position_capability_resources,
    ...
    # Cover Position Resources
    def _cover_position_capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        self._resource = AlexaPresetResource(
