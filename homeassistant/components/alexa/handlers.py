...
@HANDLERS.register((NAMESPACE_THERMOSTAT_CONTROLLER, "SetTargetTemperature"))
async def async_api_set_target_temp(
    ...
    if "targetSetpoint" in payload:
        ...
        response.add_context_property(
            {
                "name": "targetSetpoint",
                "namespace": NAMESPACE_THERMOSTAT_CONTROLLER,
...
@HANDLERS.register((NAMESPACE_THERMOSTAT_CONTROLLER, "AdjustTargetTemperature"))
...
@HANDLERS.register((NAMESPACE_THERMOSTAT_CONTROLLER, "SetThermostatMode"))
...
@HANDLERS.register((NAMESPACE_SECURITY_PANEL_CONTROLLER, "Arm"))
async def async_api_arm(
    ...
    response = directive.response(
        name="Arm.Response",
        namespace=NAMESPACE_SECURITY_PANEL_CONTROLLER,
        payload=payload,
    )
...
@HANDLERS.register((NAMESPACE_SECURITY_PANEL_CONTROLLER, "Disarm"))
...
def _set_mode_fan_direction(data: dict[str, Any], mode: str) -> str:
...
def _set_mode_fan_preset_mode(data: dict[str, Any], entity: ha.State, mode: str) -> str:
...
def _set_mode_humidifier_mode(data: dict[str, Any], entity: ha.State, mode: str) -> str:
    mode = mode.split(".")[1]
...
def _set_mode_remote_activity(data: dict[str, Any], entity: ha.State, mode: str) -> str:
...
def _set_mode_water_heater_operation_mode(
    data: dict[str, Any], entity: ha.State, mode: str
) -> str:
...
def _set_mode_cover_position(data: dict[str, Any], mode: str) -> str:
...
def _set_mode_valve_position_state(data: dict[str, Any], mode: str) -> str:
    position = mode.split(".")[1]
...
async def async_api_set_mode(
    ...
    if instance == f"{fan.DOMAIN}.{fan.ATTR_DIRECTION}":
        service = _set_mode_fan_direction(data, mode)

    # Fan preset_mode
    elif instance == f"{fan.DOMAIN}.{fan.ATTR_PRESET_MODE}":
        service = _set_mode_fan_preset_mode(data, entity, mode)

    # Humidifier mode
    elif instance == f"{humidifier.DOMAIN}.{humidifier.ATTR_MODE}":
        service = _set_mode_humidifier_mode(data, entity, mode)

    # Remote Activity
    elif instance == f"{remote.DOMAIN}.{remote.ATTR_ACTIVITY}":
        service = _set_mode_remote_activity(data, entity, mode)

    # Water heater operation mode
    elif instance == f"{water_heater.DOMAIN}.{water_heater.ATTR_OPERATION_MODE}":
        service = _set_mode_water_heater_operation_mode(data, entity, mode)

    # Cover Position
    elif instance == f"{cover.DOMAIN}.{cover.ATTR_POSITION}":
        service = _set_mode_cover_position(data, mode)

    # Valve position state
    elif instance == f"{valve.DOMAIN}.state":
        service = _set_mode_valve_position_state(data, mode)
...
def _set_range_cover_position(
    data: dict[str, Any], supported: int, range_value: int
) -> str:
...
def _set_range_fan_speed(data: dict[str, Any], supported: int, range_value: int) -> str:
    range_value = int(range_value)
    if range_value == 0:
        service = fan.SERVICE_TURN_OFF
...
def _set_range_target_humidity(data: dict[str, Any], range_value: int) -> str:
    range_value = int(range_value)
...
def _set_range_input_number_value(
    data: dict[str, Any], entity: ha.State, range_value: float
) -> str:
...
def _set_range_vacuum_fan_speed(
    data: dict[str, Any], entity: ha.State, range_value: int
) -> str:
...
async def async_api_set_range(
    ...
    if instance == f"{cover.DOMAIN}.{cover.ATTR_POSITION}":
        service = _set_range_cover_position(data, supported, range_value)

    # Cover Tilt
    elif instance == f"{cover.DOMAIN}.tilt":
        service = _set_range_cover_tilt(data, supported, range_value)

    # Fan Speed
    elif instance == f"{fan.DOMAIN}.{fan.ATTR_PERCENTAGE}":
        service = _set_range_fan_speed(data, supported, range_value)

    # Humidifier target humidity
    elif instance == f"{humidifier.DOMAIN}.{humidifier.ATTR_HUMIDITY}":
        service = _set_range_target_humidity(data, range_value)

    # Input Number Value
    elif instance == f"{input_number.DOMAIN}.{input_number.ATTR_VALUE}":
        service = _set_range_input_number_value(data, entity, range_value)

    # Number Value
    elif instance == f"{number.DOMAIN}.{number.ATTR_VALUE}":
        service = _set_range_number_value(data, entity, range_value)

    # Vacuum Fan Speed
    elif instance == f"{vacuum.DOMAIN}.{vacuum.ATTR_FAN_SPEED}":
        service = _set_range_vacuum_fan_speed(data, entity, range_value)

    # Valve Position
    elif instance == f"{valve.DOMAIN}.{valve.ATTR_POSITION}":
        service = _set_range_valve_position(data, supported, range_value)
...
def _adjust_range_cover_position(
    data: dict[str, Any], entity: ha.State, range_delta_default: bool, range_delta: int
) -> tuple[str, int]:
...
def _adjust_range_humidifier_target(
    data: dict[str, Any], entity: ha.State, range_delta_default: bool, range_delta: int
) -> tuple[str, int]:
    percentage_step = 5
    range_delta = (
...
def _adjust_range_input_number_value(
    data: dict[str, Any], entity: ha.State, range_delta: float
) -> tuple[str, float]:
...
def _adjust_range_number_value(
    data: dict[str, Any], entity: ha.State, range_delta: float
) -> tuple[str, float]:
    range_delta = float(range_delta)
    service = number.SERVICE_SET_VALUE
...
def _adjust_range_vacuum_fan_speed(
    data: dict[str, Any], entity: ha.State, range_delta: int
) -> tuple[str, Any | None]:
...
def _adjust_range_valve_position(
    data: dict[str, Any], entity: ha.State, range_delta_default: bool, range_delta: int
) -> tuple[str, int]:
    range_delta = int(range_delta * 20) if range_delta_default else int(range_delta)
    service = valve.SERVICE_SET_VALVE_POSITION
...
async def async_api_adjust_range(
    ...
    response_value: int | float | None = 0

    # Cover Position
    if instance == f"{cover.DOMAIN}.{cover.ATTR_POSITION}":
        service, response_value = _adjust_range_cover_position(
            data, entity, range_delta_default, range_delta
        )

    # Cover Tilt
    elif instance == f"{cover.DOMAIN}.tilt":
        service, response_value = _adjust_range_cover_tilt(
            data, entity, range_delta_default, range_delta
        )

    # Fan speed percentage
    elif instance == f"{fan.DOMAIN}.{fan.ATTR_PERCENTAGE}":
        service, response_value = _adjust_range_fan_speed(
            data, entity, range_delta_default, range_delta
        )

    # Humidifier target humidity
    elif instance == f"{humidifier.DOMAIN}.{humidifier.ATTR_HUMIDITY}":
        service, response_value = _adjust_range_humidifier_target(
            data, entity, range_delta_default, range_delta
        )

    # Input Number Value
    elif instance == f"{input_number.DOMAIN}.{input_number.ATTR_VALUE}":
        service, response_value = _adjust_range_input_number_value(
            data, entity, range_delta
        )

    # Number Value
    elif instance == f"{number.DOMAIN}.{number.ATTR_VALUE}":
        service, response_value = _adjust_range_number_value(data, entity, range_delta)

    # Vacuum Fan Speed
    elif instance == f"{vacuum.DOMAIN}.{vacuum.ATTR_FAN_SPEED}":
        service, response_value = _adjust_range_vacuum_fan_speed(
            data, entity, range_delta
        )

    # Valve Position
    elif instance == f"{valve.DOMAIN}.{valve.ATTR_POSITION}":
        service, response_value = _adjust_range_valve_position(
            data, entity, range_delta_default, range_delta
        )
